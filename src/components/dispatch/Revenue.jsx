...
export default function Revenue({ weekOf, driverPayroll }) {
  ...
  async function loadStops() {
    ...
    while (true) {
      const { data } = await supabase.from('daily_stops')
        .select('delivery_date, zip, pharmacy, driver_name')
  ...
  const driverProfit = useMemo(() => {
    if (!stops.length || !driverPayroll) return []
    // Group stops by driver and calculate revenue generated per driver
    const driverStops = {}
    stops.forEach(s => {
      const key = s.driver_name || 'Unknown'
      if (!driverStops[key]) driverStops[key] = []
      driverStops[key].push(s)
    })

    return Object.entries(driverStops).map(([name, dStops]) => {
      // Calculate revenue this driver generated
      const byDatePharmacy = {}
      dStops.forEach(s => {
        const key = `${s.delivery_date}|${s.pharmacy}`
        if (!byDatePharmacy[key]) byDatePharmacy[key] = { date: s.delivery_date, pharmacy: s.pharmacy, zips: {} }
        byDatePharmacy[key].zips[s.zip] = (byDatePharmacy[key].zips[s.zip] || 0) + 1
      })

      let rev = 0
      Object.values(byDatePharmacy).forEach(dp => {
        let dayPharmRev = 0
        for (const [zip, count] of Object.entries(dp.zips)) {
          const firstRate = OUTLIER_ZIPS.has(zip) ? OUTLIER_FIRST_RATE : FIRST_ORDER_RATE
          dayPharmRev += firstRate + (count - 1) * ADDITIONAL_RATE
        }
        rev += dayPharmRev
      })

      return { name, stops: dStops.length, revenue: Math.round(rev * 100) / 100 }
    }).sort((a, b) => b.revenue - a.revenue)
  }, [stops, driverPayroll])

  // Profitability per ZIP
  const zipProfit = useMemo(() => {
    if (!stops.length) return []
    const zipData = {}
    stops.forEach(s => {
      const z = s.zip
      if (!z) return
      if (!zipData[z]) zipData[z] = { zip: z, count: 0, pharmacy: s.pharmacy, dates: new Set() }
      zipData[z].count++
      zipData[z].dates.add(s.delivery_date)
    })

    return Object.values(zipData).map(z => {
      const isOutlier = OUTLIER_ZIPS.has(z.zip)
      const firstRate = isOutlier ? OUTLIER_FIRST_RATE : FIRST_ORDER_RATE
      // Revenue: first order per day at first rate, rest at additional
      const uniqueDays = z.dates.size
      const rev = (uniqueDays * firstRate) + ((z.count - uniqueDays) * ADDITIONAL_RATE)
      const avgPerStop = z.count > 0 ? rev / z.count : 0
      return {
        zip: z.zip, count: z.count, revenue: Math.round(rev * 100) / 100,
        avgPerStop: Math.round(avgPerStop * 100) / 100, isOutlier, pharmacy: z.pharmacy,
      }
    }).sort((a, b) => b.revenue - a.revenue)
  }, [stops])
  ...
  return (
    <div className="rev">
      ...
      {driverProfit.length > 0 && (
        <div className="rev__profit-section">
          <h4 className="rev__profit-title">Revenue per Driver</h4>
          <p className="rev__profit-sub">Revenue generated by each driver this week (excludes Aultman daily minimums)</p>
          <div className="rev__profit-table-wrap">
            <table className="rev__profit-table">
              <thead>
                <tr>
                  <th>Driver</th>
                  <th className="rev__th-num">Stops</th>
                  <th className="rev__th-num">Revenue Generated</th>
                  <th className="rev__th-num">$/Stop</th>
                </tr>
              </thead>
              <tbody>
                {driverProfit.map(d => (
                  <tr key={d.name}>
                    ...
                    <td className="rev__cell-num">{d.stops}</td>
                    <td className="rev__cell-num" style={{ fontWeight: 700, color: 'var(--navy)' }}>${d.revenue.toFixed(2)}</td>
                    <td className="rev__cell-num">${(d.stops > 0 ? d.revenue / d.stops : 0).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ fontWeight: 700 }}>
                  <td>Total</td>
                  <td className="rev__cell-num">{driverProfit.reduce((s, d) => s + d.stops, 0)}</td>
                  <td className="rev__cell-num" style={{ color: 'var(--navy)' }}>${driverProfit.reduce((s, d) => s + d.revenue, 0).toFixed(2)}</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
      ...
      {zipProfit.length > 0 && (
        <div className="rev__profit-section">
          <h4 className="rev__profit-title">Revenue per ZIP</h4>
          <p className="rev__profit-sub">Revenue by ZIP code this week — outlier ZIPs ($32.50) marked with star</p>
          <div className="rev__profit-table-wrap" style={{ maxHeight: 400, overflowY: 'auto' }}>
            <table className="rev__profit-table">
              <thead>
                <tr>
                  <th>ZIP</th>
                  <th className="rev__th-num">Stops</th>
                  <th className="rev__th-num">Revenue</th>
                  <th className="rev__th-num">$/Stop</th>
                </tr>
              </thead>
              <tbody>
                {zipProfit.map(z => (
                  <tr key={z.zip}>
                    <td style={{ fontWeight: 600 }}>
                      {z.zip} {z.isOutlier && <span style={{ color: '#f59e0b' }} title="Outlier ZIP ($32.50)">★</span>}
                    </td>
                    <td className="rev__cell-num">{z.count}</td>
                    <td className="rev__cell-num" style={{ fontWeight: 700, color: 'var(--navy)' }}>${z.revenue.toFixed(2)}</td>
                    <td className="rev__cell-num">${z.avgPerStop.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
